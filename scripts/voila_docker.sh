...
# recommendation.
# --entrypoint=bash: jupyter-math's default entrypoint (/docker/entrypoint.bash)
# expects oSPARC dynamic-sidecar env vars (DY_SIDECAR_PATH_INPUTS, etc.) and
# dies under `set -u` when they're unset. We don't need its jupyter-server
# startup logic — the inner script runs Voila directly.
