...
def _noise_floor_input(voila_page):
    """Return the locator for the noise floor BoundedFloatText input."""
    # Find the widget container whose label mentions "noise floor", then grab its input.
    return (
...
def test_noise_floor_widget_clamped_at_max(voila_page) -> None:
    ...
    # Restore default so later tests start from a known state.
